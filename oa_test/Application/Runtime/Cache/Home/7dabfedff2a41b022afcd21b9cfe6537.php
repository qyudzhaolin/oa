...
<div class="mainContent">
	...
	<div class="search_main">
		<a id="add_btn" class="btn btn-primary" href="<?php echo U('Permissions/add');?>">&nbsp;&nbsp;新增权限&nbsp;&nbsp;</a>
		<div class="search_input">
			<div class="col-lg-9">
			    <div class="input-group">
			      <input type="text" class="form-control" placeholder="权限名称" id="keyword" value="<?php echo ($_GET['keyword']); ?>">
			...
			<table class="table-striped table-bordered table-hover ">
				...
				<tbody>  
	                <?php if(is_array($list)): foreach($list as $k=>$vo): ?><tr class="table_bd active">
						...
						<td><?php echo ($vo["p_per_name"]); ?></td>
						<td><?php echo ($vo["controller"]); ?></td>
						<td><?php echo ($vo["action"]); ?></td>
						<td><?php if($vo["is_module"] == 1): ?>是<?php else: ?>否<?php endif; ?></td>
						<td class="operate" id="<?php echo ($vo["per_id"]); ?>">		
							...
							<div class="dropdown">
							  ...
							  <ul class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenu1">		    
									<li><a href="<?php echo U('Permissions/add');?>?per_id=<?php echo ($vo["per_id"]); ?>" class="edit">修改</a></li>
...
jQuery(document).ready(function($) {
	$("#search").click(function(){
		window.location.href="<?php echo U('Permissions/index');?>?keyword="+$("#keyword").val();
	});
	...
	$("body").on("click",".table_bd .operate .deleat",function(){
		...
		$.ajax({
			url:"<?php echo U('Permissions/delete');?>",
			type:"post",
			dataType:'json', 
			data:{'per_id':per_id},
			success:function(data){
				if(data.status == 1){
					alert("操作成功！", "<?php echo U('Permissions/index');?>?keyword="+$("#keyword").val());
				}else{
					alert(data.msg);
					return false;
				}
