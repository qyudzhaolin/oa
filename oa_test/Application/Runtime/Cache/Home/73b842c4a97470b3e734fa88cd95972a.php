...
<div class="mainContent">
	...
	<div class="search_main">
		<a id="add_btn" class="btn btn-primary" href="<?php echo U('Teambuild/add');?>">&nbsp;&nbsp;新增TB申请&nbsp;&nbsp;</a>
		<a id="add_btn" class="btn btn-primary" href="<?php echo U('Tbrecouped/index');?>" style="margin-left:10px">&nbsp;&nbsp;TB报销单&nbsp;&nbsp;</a>
			...
			<table class="table-striped table-bordered table-hover ">
				...
				<tbody>  
	                <?php if(is_array($list)): foreach($list as $k=>$vo): ?><tr class="table_bd active">
						...
						<td><?php echo ($vo["tb_date"]); ?></td>
						<td><?php echo ($vo["result_info"]); ?></td>
						<td><?php echo ($vo["crt_time"]); ?></td>
						<td class="operate" id="<?php echo ($vo["t_id"]); ?>">
						    <?php if($vo["action"] == 'add'): ?><a href="<?php echo U('Teambuild/add');?>?id=<?php echo ($vo["t_id"]); ?>" class="edit">修改</a>
								<a href="javascript:;" class="deleat">删除</a>
						    <?php else: ?>	
								<a href="<?php echo U('Teambuild/info');?>?id=<?php echo ($vo["t_id"]); ?>" class="edit">详情</a><?php endif; ?>
							<?php if(check_permission_left('financial_approve', 'finance') && session('lvl_id')!=1){ ?>
							<a href="javascript:;" class="deleat">删除</a>
							<?php } ?>
							<?php if(($user_id) == $vo["crt_user_id"]): if($vo["result"] == '1'): ?><a href="<?php echo U('Tbrecouped/add');?>?t_id=<?php echo ($vo["t_id"]); ?>" class="edit">生成报销单</a><?php endif; endif; ?>
						</td>
...
jQuery(document).ready(function($) {
	...
	$("body").on("click",".table_bd .operate .deleat",function(){
		...
		$.ajax({
			url:"<?php echo U('Teambuild/delete');?>",
			type:"post",
			dataType:'json', 
			data:{'t_id':t_id},
			success:function(data){
				if(data.status == 1){
					alert("操作成功！", "<?php echo U('Teambuild/index');?>");
				}else{
					alert(data.msg);
					return false;
				}
	...
	$("#search").click(function(){
		window.location.href="<?php echo U('Teambuild/index');?>?keyword="+$("#keyword").val()+"&way="+$("#way_select").val()+"&start_date="+$("#start_date").val()+"&end_date="+$("#end_date").val();
	})
	
	$('#export').click(function () {
		window.location.href="<?php echo U('Teambuild/export');?>?keyword="+$("#keyword").val()+"&way="+$("#way_select").val()+"&start_date="+$("#start_date").val()+"&end_date="+$("#end_date").val();
    });
