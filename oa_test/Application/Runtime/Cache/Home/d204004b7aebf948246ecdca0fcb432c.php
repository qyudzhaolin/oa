...
<div class="mainContent">
	...
	<div class="search_main form-inline">
		...
		<div style="float:left;">			
			<a id="add_btn" class="btn btn-primary" href="<?php echo U('Recouped/add');?>">&nbsp;&nbsp;新增项目报销单&nbsp;&nbsp;</a>
			<?php if($is_use_mj): ?><a style="margin-left:20px" id="add_btn" class="btn btn-primary" href="<?php echo U('Recouped/mj_add');?>">&nbsp;&nbsp;新增媒介报销单&nbsp;&nbsp;</a><?php endif; ?>
		</div>
		<!-- 左 -->	
		<!-- 右 -->	
		<div style="float:right;">			
		    ...
		    <div class="form-group">
		    	<div class="input-group">
			      <input type="text" class="form-control" id="keyword" value="<?php echo ($_GET['keyword']); ?>" placeholder="项目名称/申请人姓名">
			      <span class="input-group-btn">
			        <button class="btn btn-success" type="button" id="search">搜索</button>
			      </span>
			    </div>
		    </div>
			<?php if(check_permission_left('financial_approve', 'recouped_export')){ ?>
			...
			<table class="table-striped table-bordered table-hover ">
				...
				<tbody>  
	                <?php if(is_array($list)): foreach($list as $k=>$vo): ?><tr class="table_bd active">
						...
						<td class="operate" id="<?php echo ($vo["rec_id"]); ?>">
						    <?php if($vo["action"] == 'add'): if($vo["mj_ex_id"] == 0): ?><a href="<?php echo U('Recouped/add');?>?id=<?php echo ($vo["rec_id"]); ?>" class="edit">修改</a>
							    <?php else: ?>	
							    <a href="<?php echo U('Recouped/mj_add');?>?id=<?php echo ($vo["rec_id"]); ?>" class="edit">修改</a><?php endif; ?>
								<a href="javascript:;" class="deleat">删除</a>
						    <?php else: ?>
						    	<?php if($vo["mj_ex_id"] == 0): ?><a href="<?php echo U('Recouped/info');?>?id=<?php echo ($vo["rec_id"]); ?>" class="edit">详情</a>
							    <?php else: ?>		
								<a href="<?php echo U('Recouped/mj_info');?>?id=<?php echo ($vo["rec_id"]); ?>" class="edit">详情</a><?php endif; endif; ?>
...
jQuery(document).ready(function($) {
	...
	$("body").on("click",".table_bd .operate .deleat",function(){
		...
		$.ajax({
			url:"<?php echo U('Recouped/delete');?>",
			type:"post",
			dataType:'json', 
			data:{'rec_id':rec_id},
			success:function(data){
				if(data.status == 1){
					alert("操作成功！", "<?php echo U('Recouped/index');?>");
				}else{
					alert(data.msg);
					return false;
				}
	...
	$("#search").click(function(){
		window.location.href="<?php echo U('Recouped/index');?>?keyword="+$("#keyword").val()+"&way="+$("#way_select").val()+"&start_date="+$("#start_date").val()+"&end_date="+$("#end_date").val();
	})
	$("#export").click(function(){
		window.location.href="<?php echo U('Recouped/export');?>?keyword="+$("#keyword").val()+"&way="+$("#way_select").val()+"&start_date="+$("#start_date").val()+"&end_date="+$("#end_date").val();
	})
});
</script></div>
</div>
