...
<div class="mainContent">
	...
	<div class="search_main">
		<a id="add_btn" class="btn btn-primary" href="<?php echo U('Seal/add');?>">&nbsp;&nbsp;新增用章申请&nbsp;&nbsp;</a>
		<div class="search_input" style="width:1000px;margin-right:10px;">
			<?php if($deparse_id == 1 OR $is_admin != ''): ?><a id="export" class="btn btn-primary" style="float:right;margin-top: 8px;">&nbsp;&nbsp;导出&nbsp;&nbsp;</a><?php endif; ?>
			<div class="col-lg-9" style="width:300px">
			    <div class="input-group">
			      <input type="text" class="form-control" id="keyword" value="<?php echo ($_GET['keyword']); ?>" placeholder="请输入申请人姓名">
			...
			<table class="table-striped table-bordered table-hover ">
				...
				<tbody>  
	                <?php if(is_array($list)): foreach($list as $k=>$vo): ?><tr class="table_bd active">
						...
						<td><?php if($vo["s_type"] == '1'): ?>盖章申请<?php else: ?>借章申请<?php endif; ?></td>
						<td><?php if($vo["se_type"] == '1'): ?>公章<?php elseif($vo["se_type"] == '2'): ?>财务章<?php elseif($vo["se_type"] == '3'): ?>法人章<?php elseif($vo["se_type"] == '4'): ?>发票章<?php elseif($vo["se_type"] == '5'): ?>合同章<?php endif; ?></td>
						<td><?php if($vo["c_type"] == '1'): ?>榕智<?php elseif($vo["c_type"] == '2'): ?>炜旻<?php elseif($vo["c_type"] == '3'): ?>络昕<?php endif; ?></td>
						<td><?php echo ($vo["use_num"]); ?></td>
						<td><?php echo ($vo["out_date"]); ?></td>
						<td><?php echo ($vo["back_date"]); ?></td>
						<td><?php echo ($vo["result_info"]); ?></td>
						<td><?php echo ($vo["crt_time"]); ?></td>
						<td class="operate" id="<?php echo ($vo["se_id"]); ?>">
						    <?php if($vo["action"] == 'add'): ?><a href="<?php echo U('Seal/add');?>?id=<?php echo ($vo["se_id"]); ?>" class="edit">修改</a>
								<a href="javascript:;" class="deleat">删除</a>
						    <?php else: ?>	
								<a href="<?php echo U('Seal/info');?>?id=<?php echo ($vo["se_id"]); ?>" class="edit">详情</a><?php endif; ?>
	...
	<div class="Pagination"><?php echo ($page); ?></div>	<!-- E-表格部分 -->
</div>
<!-- E-页面主要部分 -->
<script>
jQuery(document).ready(function($) {
	...
	$("#search").click(function(){
		window.location.href="<?php echo U('Seal/index');?>?keyword="+$("#keyword").val()+"&way="+$("#way_select").val()+"&start_date="+$("#start_date").val()+"&end_date="+$("#end_date").val();
	})
	
	$('#export').click(function () {
		window.location.href="<?php echo U('Seal/export');?>?keyword="+$("#keyword").val()+"&way="+$("#way_select").val()+"&start_date="+$("#start_date").val()+"&end_date="+$("#end_date").val();
    });
