...
<div class="mainContent">
	...
	<div class="search_main">
		<a id="add_btn" class="btn btn-primary" href="<?php echo U('Pfrecouped/add');?>">&nbsp;&nbsp;新增个人报销单&nbsp;&nbsp;</a>
			...
			<table class="table-striped table-bordered table-hover ">
				...
				<tbody>  
	                <?php if(is_array($list)): foreach($list as $k=>$vo): ?><tr class="table_bd active">
						...
						<td class="operate" id="<?php echo ($vo["pf_id"]); ?>">
						    <?php if($vo["action"] == 'add'): ?><a href="<?php echo U('Pfrecouped/add');?>?id=<?php echo ($vo["pf_id"]); ?>" class="edit">修改</a>
								<a href="javascript:;" class="deleat">删除</a>
						    <?php else: ?>	
								<a href="<?php echo U('Pfrecouped/info');?>?id=<?php echo ($vo["pf_id"]); ?>" class="edit">详情</a><?php endif; ?>
...
jQuery(document).ready(function($) {
	...
	$("body").on("click",".table_bd .operate .deleat",function(){
		...
		var pf_id = $(this).parent().attr('id');
		
		$.ajax({
			url:"<?php echo U('Pfrecouped/delete');?>",
			type:"post",
			dataType:'json', 
			data:{'pf_id':pf_id},
			success:function(data){
				if(data.status == 1){
					alert("操作成功！", "<?php echo U('Pfrecouped/index');?>");
				}else{
					alert(data.msg);
					return false;
				}
	...
	$("#search").click(function(){
		//$('#searchform').attr('action',"<?php echo U('Pfrecouped/index');?>");
        //$('#searchform').submit();
		window.location.href="<?php echo U('Pfrecouped/index');?>?keyword="+$("#keyword").val()+"&way="+$("#way_select").val()+"&start_date="+$("#start_date").val()+"&end_date="+$("#end_date").val();
	})
	
	$('#export').click(function () {
        /*$('#searchform').attr('action',"<?php echo U('Pfrecouped/export');?>");
        $('#searchform').submit();*/
		window.location.href="<?php echo U('Pfrecouped/export');?>?keyword="+$("#keyword").val()+"&way="+$("#way_select").val()+"&start_date="+$("#start_date").val()+"&end_date="+$("#end_date").val();
    });
