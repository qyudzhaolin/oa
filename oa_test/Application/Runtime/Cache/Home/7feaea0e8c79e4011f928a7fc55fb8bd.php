...
<div class="mainContent">
	...
	<div class="search_main form-inline">
		...
		<div style="float:left;">
			<a id="add_btn" class="btn btn-primary" href="<?php echo U('Refund/add');?>">&nbsp;&nbsp;新增还款单&nbsp;&nbsp;</a>
		</div>
		<!-- 左 -->	
		<!-- 右 -->	
			...
			<table class="table-striped table-bordered table-hover ">
				...
				<tbody>  
	                <?php if(is_array($list)): foreach($list as $k=>$vo): ?><tr class="table_bd active">
						...
						<td class="operate" id="<?php echo ($vo["ref_id"]); ?>">
						    <?php if($lvl_id == '1'): ?><a href="<?php echo U('Refund/add');?>?id=<?php echo ($vo["ref_id"]); ?>" class="edit">修改</a>
								<a href="javascript:;" class="deleat">删除</a>
						    <?php else: ?>	
							    <?php if(($user_id) == $vo["crt_user_id"]): ?><a href="<?php echo U('Refund/add');?>?id=<?php echo ($vo["ref_id"]); ?>" class="edit">修改</a>
									<a href="javascript:;" class="deleat">删除</a>
								<?php else: ?>
									<a href="<?php echo U('Refund/info');?>?id=<?php echo ($vo["ref_id"]); ?>" class="edit">详情</a><?php endif; endif; ?>
						</td>
...
jQuery(document).ready(function($) {
	...
	$("#search").click(function(){
		window.location.href="<?php echo U('Refund/index');?>?keyword="+$("#keyword").val();
	})
});
</script></div>
</div>
