	...
	<!--E-头部 -->
		...
		<div class="mainContent">
			...
			<div class="search_main">
				<form action="<?php echo U('UserHr/index');?>" method="post">
					
					<div id="select" class="col-xs-1">
						<input type="text" class="form-control" placeholder="名称" name="name" value="<?php echo ($search["name"]); ?>">
					</div>
					<div id="select" class="col-xs-1">
						<input type="text" class="form-control" placeholder="电话" name="phone" value="<?php echo ($search["phone"]); ?>">
					</div>
					<div id="select" class="col-xs-1">
						<input type="text" class="form-control" placeholder="面试岗位" name="job" value="<?php echo ($search["job"]); ?>">
					</div>
					<div id="select" class="mg-right-15">
						<button class="btn btn-primary" type="submit" id="search">查询</button>
					</div>
					<?php if($is_admin == 1 || $depart_id == 12 || $depart_id == 60): ?><div id="select" class="mg-right-15">
						<a id="" class="btn btn-primary" href="<?php echo U('UserHr/add');?>">&nbsp;&nbsp;新增招聘信息&nbsp;&nbsp;</a>
						</div><?php endif; ?>
				</form>
			</div>
			<!-- S-表格部分 -->
				...
				<table class="table table-striped table-bordered table-hover ">
					<thead>
						<tr id="table_head">
							...
							<th width="10%">简历</th>
							<th width="10%">期望薪资</th>
							<?php if($add != ''): ?><th width="10%">操作</th><?php endif; ?>
						</tr>
					</thead>
					<tbody>  
						<?php if(is_array($list)): $key = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($key % 2 );++$key;?><tr class="table_bd active">
							...
							<td>
								<a href="<?php echo ($vo["file_url"]); ?>" target="_black">简历附件</a>					
							</td>
							<td>
								<?php echo ($vo["pre_salary"]); ?>
							</td>
							<?php if($add != ''): ?><td class="operate">
								<?php if($is_admin == 1 || $depart_id == 12 || $depart_id == 60): ?><a href="<?php echo U('UserHr/add',array('id'=>$vo['id']));?>" class="edit">修改</a>
								<a href="javascript:void(0)" value="<?php echo ($vo["id"]); ?>" url="<?php echo U('UserHr/del');?>" class="deleat">删除</a><?php endif; ?>
								<a href="<?php echo U('UserHr/detail',array('id'=>$vo['id'],'type'=>1));?>" class="edit">查看</a>
							</td><?php endif; ?>
