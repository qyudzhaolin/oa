...
use Think\Model;
class TbApproveModel extends Model{
    
    protected $tableName = 'tb_approve';
     ...
     * @author maojingjing
     */
    public function getApproveSchedule($t_id){
        if(empty($t_id))return false;
        $approve = $this->get_onebywhere("t_id=$t_id and is_del=1");
        return $approve;
    }
    ...
    public function get_whereaprrove($user_id){
        return "select t_id from max_tb_approve where is_del=1 and  (aprv_user_id1 = $user_id or aprv_user_id2 = $user_id or aprv_user_id3 = $user_id)";
    }
