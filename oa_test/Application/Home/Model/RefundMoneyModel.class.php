...
use Think\Model;
class RefundMoneyModel extends Model{
    
    protected $tableName = 'refund_money';
    ...
    public function get_one($id){
        ...
        return $this->where("rm_id = $id")->find();
    }
    ...
    public function get_moneyandcost_list($obj_id, $obj_type=1){
        ...
        $sql = "select b.* from max_money a inner join max_cost_type b on b.id=a.cost_id where a.obj_id=$obj_id and obj_type=$obj_type and a.is_del=1";
    ...
    public function get_array_list($obj_id, $obj_type=1){
        if(empty($obj_id)||!is_numeric($obj_id))return false;
        $moneys = $this->get_list("obj_id=$obj_id and obj_type=$obj_type and is_del=1");  
        $arr = null;
